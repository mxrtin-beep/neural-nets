...
// inverted format with a positive bmpInfoHeader_biHeight value.
   ...
   /**
    * Converts a file of 10201 ints to a 101x101 BMP file.
    * @param pelsFile   The name of the file of ints.
    */
   public int[][] pelsFileToArray(String pelsFile)
   {
      // Calculates the dimensions of the bmp file based on the squareroot of
      // the number of lines in the pels file.
      int dim = (int) Math.sqrt(countLines(pelsFile));
      assert (dim == 101);
      
      int[][] pels = fileToArray(pelsFile, dim, dim);
      return pels;
   }
   
   /**
    * Counts the number of lines in a document.
    * @param filename
    * @return
    */
   public int countLines(String filename)
   {
      try
      {
         BufferedReader counter1 = new BufferedReader(new FileReader(filename));
    ...
    */
   public int[][] fileToArray(String s, int dimA, int dimB)
   {
      //String s = "/Users/martinbourdev/IdeaProjects/Perceptron1/text/input";
      try
      {
         int[][] pels = new int[dimA][dimB];
         ...
         {
            pels[indexY][indexX] = Integer.parseInt(counter2.readLine());
            if(indexX < dimA - 1)
            {
               indexX++;
            }
   ...
   /**
    * Writes a double array of ints to a file.
    * @param filename   The desired filename.
    * @param pels       The double array of ints.
    */
   public void arrayToFile(String filename, double[][] pels)
   {
      Iterator<String> itr = arrayToIterator(pels);
      FileUtil.saveFile(filename, itr);
   }
   
   /**
    * Converst a 2D array of ints to a String iterator.
    * @param pels The 2D array of ints.
    * @return     pels in the form of a String iterator.
    */
   public Iterator<String> arrayToIterator(double [][] pels)
   {
      ...
      for(int i = 0; i < pels.length; i++)
      {
         ...
         {
            list.add(Double.toString(pels[i][j]));
         }
      }
   ...
   public void writeBMPFile(String outFileName, int[][] pels)
   {
      int i, j, k;
      int numberOfColors;
      int pel;
...
// there is no color table for this true color image, so write out the pels
   ...
   public int[][] getPels(String inFileName)
   {
      String outFileName;
      int i, j, k;
      int numberOfColors;
...
// there is no color table for this true color image, so write out the pels
      ...
      String inputFile = "pelsFile";
      int[][] pels;
      String bmpFileName = "outFile.bmp";
      //dibdumper.pelsToFile("pelsFile", imageArray);
      pels = dibdumper.pelsFileToArray(inputFile);
      dibdumper.writeBMPFile("clown.bmp", pels);
